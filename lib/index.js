...
function run(result) {
    if (result.param && ("action" in result.param)) {
        ...
        action_1.default[result.param.action](result.param, result);
    }
    else {
        debug("Unknow what to run,fallback to copy");
        action_1.default.copy(null, result);
    }
    cache.clear();
}
exports.run = run;
        ...
        return asyncify(cmds)
            ...
            if (p) {
                ret = (yield p(next.args || {}, pre))
                    .map(item => (Object.assign({}, item, { icon: item.icon || lookUpIcon(next.cmd) })));
