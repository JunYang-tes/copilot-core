...
async function parse(obj, fileName, name) {
    const processors = {};
    if (obj.init && util.isFunction(obj.init)) {
        await obj.init(config_1.getConfig(name({ fileName, funName: "init" })));
    }
...
async function load({ dir = __dirname, name = ({ fileName, funName }) => `buildin.${fileName}.${funName.replace("bound ", "")}` }) {
    ...
    let modules = await asyncify(files)
        ...
        if (/\.js$/.test(file) && file !== "index.js") {
            ...
            return (await util_2.stat(`${dir}/${file}`)).isDirectory();
        }
    })
        .map((file) => {
        try {
