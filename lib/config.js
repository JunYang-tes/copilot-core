...
loadConfig();
function loadConfig4ExProcessors(path) {
    debug("merge external processor's config");
    path.map(p => {
        p = util_1.default.path(p);
        try {
            return fs_1.readdirSync(`${p}`)
                .filter(f => fs_1.statSync(`${p}/${f}`)
                .isDirectory() && fs_1.existsSync(`${p}/${f}/config.yaml`))
        ...
        p.forEach(configFile => {
            try {
                let c = yaml.safeLoad(fs_1.readFileSync(configFile));
...
function getConfigByKeys(...params) {
...
exports.getConfigByKeys = getConfigByKeys;
